...
export default function AppShell({ children }: { children: React.ReactNode }) {
  ...
  const isAuthRoute = segments[1] === 'auth';
  ...
  // Protección de rutas (omite rutas de auth)
  useEffect(() => {
    if (!isAuthRoute && !loading && !contextLoading && !user) {
      console.log('❌ No user found, redirecting to login');
      router.push(`/${currentLocale}/auth/login`);
    }
  }, [isAuthRoute, loading, contextLoading, user, router, currentLocale]);
  ...
  // Loading state (omite skeleton en rutas de auth)
  if (!isAuthRoute && (loading || contextLoading)) {
    return (
      <div className="flex h-screen items-center justify-center bg-gradient-to-br from-gray-50 to-gray-100 dark:from-gray-950 dark:to-gray-900">
        <div className="text-center">
          ...
          <p className="text-gray-600 dark:text-gray-400 font-medium">Cargando MediStock...</p>
        </div>
      </div>
    );
  }

  // En rutas de auth, renderiza contenido directamente sin Header/Sidebar
  if (isAuthRoute) {
    return <>{children}</>;
  }

  // Si no hay usuario y no es ruta de auth, no renderiza contenido
  if (!user) return null;
      ...
      <div className="flex h-screen overflow-hidden bg-gray-50 dark:bg-gray-950">
        ...
        >
          <HospitalSidebar
            ...
            onPageChange={(page) => router.push(`/${currentLocale}/${page}`)}
            LinkComponent={Link}
          />
        </aside>
